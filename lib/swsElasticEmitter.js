...
swsElasticEmitter.prototype.initialize = function (swsOptions) {
    ...
    this.elasticAuth = swsOptions[swsUtil.supportedOptions.elasticsearch_auth];
    this.elasticCA = swsOptions[swsUtil.supportedOptions.elasticsearch_ca];
    this.elasticCert = swsOptions[swsUtil.supportedOptions.elasticsearch_cert];
    this.elasticKey = swsOptions[swsUtil.supportedOptions.elasticsearch_key];
...
swsElasticEmitter.prototype.initTemplate = function(rrr) {
    ...
    var headers = this.elasticAuth ? {
      'Authorization': this.elasticAuth
    } : {};

    var agentOptions = {};
    if (this.elasticCA) agentOptions.ca = fs.readFileSync(this.elasticCA);
    if (this.elasticCert) agentOptions.cert = fs.readFileSync(this.elasticCert);
    if (this.elasticKey) agentOptions.key = fs.readFileSync(this.elasticKey);

    // Check if there is a template
    var templateURL = this.elasticURL+'/_template/template_api';
    request.get({url:templateURL, json:true, headers: headers, agentOptions: agentOptions}, function (error, response, body) {
        if(error) {
            ...
            if(initializeNeeded){
                request.put({url:templateURL,json:indexTemplate, headers: headers}, function (error, response, body) {
                    if(error) {
...
swsElasticEmitter.prototype.flush = function(){
    ...
    if (this.elasticAuth) options.headers['Authorization'] = this.elasticAuth;
