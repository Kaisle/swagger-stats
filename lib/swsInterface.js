...
module.exports = {
    ...
    getMiddleware: function (options) {
        ...
        if (options && options.managementApp) {
          options.managementApp.use(trackingMiddleware);
        }
        ...
        function trackingMiddleware(req, res, next) {
            ...
            if (!options || !options.managementApp) handleRequest(req, res);

            return next();
        };

        function handleRequestWrapper(req, res, next) {
          handleRequest(req, res);
          return next();
        }

        return options && options.managementApp ? handleRequestWrapper : trackingMiddleware;
